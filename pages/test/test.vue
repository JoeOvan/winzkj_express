		...
		<view class="content">
			...
			<el-form class="form" ref="form" :model="form" label-width="80px">
			  ...
			  <el-form-item label="姓名">
			    <el-input v-model="receivedName"></el-input>
			  </el-form-item>
			  <el-form-item label="手机号码">
			    <el-input v-model="receivedPhone"></el-input>
			  </el-form-item>
			  <el-form-item label="配送地址">
				...
				<el-select class="select-item" v-model="form.address.level" placeholder="请选择宿舍楼..." @change="getCampusListPlate">
				  <el-option
				...
				<el-select class="select-item" v-model="form.address.plate" placeholder="请选择宿舍号...">
				  <el-option
			  ...
			  <view class="tips"><text class="sign">*</text>如包裹过大过重（如大件行李、大件饮料等），平台有权利联系您，追加服务费用。</view>
	...
	export default {
		data() {
			return {
				...
				orderShippingTime: '',
				expressData: {},
				// isInitLevel: 0,  //初始化Level
				// isInitPlate: 0,  //初始化Plate
			}
		},
		async onLoad() {
		...
		methods: {
			...
			async getCampusListLevel(obj) {
				...
				// //如果不是初始化则重置
				// if(this.isInitLevel !== 0) {
					
				// 	//重新选择一级类目时重置二级三级类目
					this.form.address.level = {};
				// 	this.form.levelOptions = [];
			...
			async submitForm() {
				...
				//请选择配送地址-校区...!
				if(!this.form.address.area || this.form.address.area == '') return uni.showToast({title:'请选择配送地址-校区...!', icon: 'none', duration: 2000});
				//请选择配送地址-宿舍楼...!
				if(!this.form.address.level || this.form.address.level == '') return uni.showToast({title:'请选择配送地址-宿舍楼...!', icon: 'none', duration: 2000});
				//请选择配送地址-宿舍号...!
				if(!this.form.address.plate || this.form.address.plate == '') return uni.showToast({title:'请选择配送地址-宿舍号...!', icon: 'none', duration: 2000});
				
				this.form.receivedAddress = this.form.address.area.label + "-" + this.form.address.level.label + "-" + this.form.address.plate.label;
